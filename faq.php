<!DOCTYPE html>
<html lang="en">

<head>
    <?php include("include/head.php"); ?>
</head>

<body>
    <!--Header Start-->
    <?php include("include/header.php"); ?>
    <!--Header End-->
    <!-- banner -->
    <div class="banner inner-banner">
        <div class="bg-image">
            <img src="assets/images/blog-bg.webp" alt="img" width="" height="">
        </div>
        <div class="container">
            <div class="banner-info">
                <div class="shape">
                    <img src="./assets/images/banner-shape.webp" alt="" width="" height="">
                </div>
                <h1 class="banner-title h1" data-gsap="fadeUp">FAQs</h1>
                <div class="breadcrumb" data-gsap="fadeUp">
                    <ul>
                        <li><a href="#">Home</a></li>
                        <li>FAQs</li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    <!-- banner -->

    <div class="faq-wrap pt-160 pb-160">
        <div class="container">
            <div class="row gap-x-30">
                <div class="col-lg-4">
                    <div class="title-wrap" data-gsap="fadeUp">
                        <h2 class="h2">We offer a wide
                            range of services
                            to meet your
                            needs, including:
                        </h2>
                    </div>
                </div>
                <div class="col-lg-7 offset-lg-1">
                    <div class="faq-wrap" data-gsap="fadeUp">
                        <div class="accordion" id="homeFaq">
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button type="button" data-bs-toggle="collapse" data-bs-target="#homeFaq-1">
                                        Do you accept insurance?
                                        <span class="icon"></span></button>
                                </h2>
                                <div id="homeFaq-1" class="accordion-collapse collapse show" data-bs-parent="#homeFaq">
                                    <div class="accordion-body">
                                        <p>Trust deed investing is a form of real estate investing where an investor
                                            loans money to a
                                            borrower secured by a deed of trust on a property. The investor becomes the
                                            lender and earns a fixed rate of return on the loan.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#homeFaq-2">
                                        What types of counseling do you offer? <span class="icon"></span>
                                    </button>
                                </h2>
                                <div id="homeFaq-2" class="accordion-collapse collapse" data-bs-parent="#homeFaq">
                                    <div class="accordion-body">
                                        <p>Trust deed investing is a form of real estate investing where an investor
                                            loans money to a
                                            borrower secured by a deed of trust on a property. The investor becomes the
                                            lender and earns a fixed rate of return on the loan.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#homeFaq-3">
                                        How do I know if therapy is right for me?
                                        <span class="icon"></span>
                                    </button>
                                </h2>
                                <div id="homeFaq-3" class="accordion-collapse collapse" data-bs-parent="#homeFaq">
                                    <div class="accordion-body">
                                        <p>Trust deed investing is a form of real estate investing where an investor
                                            loans money to a
                                            borrower secured by a deed of trust on a property. The investor becomes the
                                            lender and earns a fixed rate of return on the loan.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#homeFaq-4">
                                        What types of counseling do you offer?
                                        <span class="icon"></span>
                                    </button>
                                </h2>
                                <div id="homeFaq-4" class="accordion-collapse collapse" data-bs-parent="#homeFaq">
                                    <div class="accordion-body">
                                        <p>Trust deed investing is a form of real estate investing where an investor
                                            loans money to a
                                            borrower secured by a deed of trust on a property. The investor becomes the
                                            lender and earns a fixed rate of return on the loan.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#homeFaq-5">
                                        How do I know if therapy is right for me?

                                        <span class="icon"></span>
                                    </button>
                                </h2>
                                <div id="homeFaq-5" class="accordion-collapse collapse" data-bs-parent="#homeFaq">
                                    <div class="accordion-body">
                                        <p>Trust deed investing is a form of real estate investing where an investor
                                            loans money to a
                                            borrower secured by a deed of trust on a property. The investor becomes the
                                            lender and earns a fixed rate of return on the loan.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#homeFaq-6">
                                        What types of counseling do you offer?

                                        <span class="icon"></span>
                                    </button>
                                </h2>
                                <div id="homeFaq-6" class="accordion-collapse collapse" data-bs-parent="#homeFaq">
                                    <div class="accordion-body">
                                        <p>Trust deed investing is a form of real estate investing where an investor
                                            loans money to a
                                            borrower secured by a deed of trust on a property. The investor becomes the
                                            lender and earns a fixed rate of return on the loan.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#homeFaq-7">
                                        How do I know if therapy is right for me?


                                        <span class="icon"></span>
                                    </button>
                                </h2>
                                <div id="homeFaq-7" class="accordion-collapse collapse" data-bs-parent="#homeFaq">
                                    <div class="accordion-body">
                                        <p>Trust deed investing is a form of real estate investing where an investor
                                            loans money to a
                                            borrower secured by a deed of trust on a property. The investor becomes the
                                            lender and earns a fixed rate of return on the loan.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#homeFaq-8">
                                        What types of counseling do you offer?
                                        <span class="icon"></span>
                                    </button>
                                </h2>
                                <div id="homeFaq-8" class="accordion-collapse collapse" data-bs-parent="#homeFaq">
                                    <div class="accordion-body">
                                        <p>Trust deed investing is a form of real estate investing where an investor
                                            loans money to a
                                            borrower secured by a deed of trust on a property. The investor becomes the
                                            lender and earns a fixed rate of return on the loan.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#homeFaq-9">
                                        How do I know if therapy is right for me?
                                        <span class="icon"></span>
                                    </button>
                                </h2>
                                <div id="homeFaq-9" class="accordion-collapse collapse" data-bs-parent="#homeFaq">
                                    <div class="accordion-body">
                                        <p>Trust deed investing is a form of real estate investing where an investor
                                            loans money to a
                                            borrower secured by a deed of trust on a property. The investor becomes the
                                            lender and earns a fixed rate of return on the loan.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="faq-testimonials-wrap pt-170 pb-100"
        style="background-image: url(./assets/images/home-faq-testimonial-bg.webp);">
        <div class="container">
            <div class="testimonial-content-box">
                <div class="title-wrap" data-gsap="fadeUp">
                    <div class="sub-title text-center">Testimonials</div>
                </div>
                <div class="testimonial-box large" data-gsap="fadeUp">
                    <div class="icon-box">
                        <svg class="icon" width="90" height="90">
                            <use xlink:href="./assets/images/svgsprit.svg#qoute-icon"></use>
                        </svg>
                    </div>
                    <p>I’ve never felt more supported and understood
                        than I do with my counselor at Inspiring Hope.
                        The non-judgmental atmosphere and
                        personalized care are incredible!</p>
                    <div class="name">- Jessica R.</div>
                </div>
                <div class="btn-wrap text-center pt-50" data-gsap="fadeUp">
                    <a href="#" class="btn">Read More</a>
                </div>
            </div>
        </div>
    </div>

    <!-- home-blog -->
    <div class="home-blog pt-160 pb-160">
        <div class="shape">
            <img src="./assets/images/home-blog-dot.webp" alt="shape" width="" height="">
        </div>
        <div class="container">
            <div class="row gap-x-30">
                <div class="col-lg-5">
                    <div class="content-box">
                        <div class="title-wrap">
                            <div class="sub-title" data-gsap="fadeUp">Our Blog</div>
                            <div class="sub-text text-lg" data-gsap="fadeUp">
                                <p>Stay informed with our latest articles
                                    on mental health, self-care, and
                                    therapy tips. Our blog offers valuable
                                    insights and resources to help you on
                                    your journey.
                                </p>
                            </div>
                            <div class="btn-wrap" data-gsap="fadeUp">
                                <a href="#" class="btn">Read More</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-7">
                    <ul class="list">
                        <li>
                            <div class="box blog-box" data-gsap="fadeUp">
                                <div class="img img-cover">
                                    <img src="./assets/images/blog-1.webp" alt="img" width="" height="">
                                </div>
                                <div class="info">
                                    <h3 class="h5"><a href="#">Lorem ipsum dolor sit amet,
                                            consectetur adipiscing elit</a></h3>
                                    <p>Donec commodo ultrices turpis, eu
                                        posuere urna suscipit at. Praesent
                                        lacus nisi, congue nec erat a</p>
                                    <div class="btn-wrap">
                                        <a href="#" class="btn-link primary">Read More</a>
                                    </div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="box blog-box" data-gsap="fadeUp">
                                <div class="img img-cover">
                                    <img src="./assets/images/blog-1.webp" alt="img" width="" height="">
                                </div>
                                <div class="info">
                                    <h3 class="h5"><a href="#">Lorem ipsum dolor sit amet,
                                            consectetur adipiscing elit</a></h3>
                                    <p>Donec commodo ultrices turpis, eu
                                        posuere urna suscipit at. Praesent
                                        lacus nisi, congue nec erat a</p>
                                    <div class="btn-wrap">
                                        <a href="#" class="btn-link primary">Read More</a>
                                    </div>
                                </div>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    <!-- home-blog -->

    <!--Footer Start-->
    <?php include("include/footer.php"); ?>
    <?php include("include/script.php"); ?>
    <!--Footer End-->
</body>

</html>